...
heart::heart(real x0, real y0, real phi, real stretch, real x_scale, real y_scale)
...
void heart::calc_bounds() {
  ...
  //bottom_bound =
  //    (a - std::sqrt(a * a - 4_r * x0 * stretch * sin_phi)) / (a * sin_phi_sqrd) -
...
void heart::draw(window* win) {
  ...
  int bottom_bound_cropped_mapped = map_to_screen_height(top_bound_cropped, win);
  int top_bound_cropped_mapped    = map_to_screen_height(bottom_bound_cropped, win);
  int right_bound_cropped_mapped  = map_to_screen_width(right_bound_cropped, win);
  ...
  auto max_bound_cropped = std::max(right_bound_cropped, std::abs(left_bound_cropped));
    ...
    for (auto curr_pix_y = bottom_bound_cropped_mapped;
      ...
      if (is_inside) {
        ...
        if (left_mirror >= 0 && left_mirror < win->_width) {
          pixls[curr_pix_y * (surf->pitch / 4) + left_mirror] = pixls[curr_pix_y * (surf->pitch / 4) + curr_pix_x];
        }
      }
    }
  }
